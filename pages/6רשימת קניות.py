...
import os
import base64
from fpdf import FPDF  # Make sure to have this installed: pip install fpdf2
...
def show_shopping_list_page():    
    ...
    if selected == "צור רשימה לכלב":
        ...
        with col2:
            if st.button("צור רשימה"):
                if st.session_state['dog'] is not None:
                    st.session_state["step"] = 1
                    st.session_state["shopping list"] = None
                    st.session_state["short list"] = None
                else:
...
def create_list(dog):
    ...
    if df["Size"][0] not in ['XS', 'S', 'M', 'L', 'XL']:
        st.warning('Dog has NO size!')    
    for c in categories:
        if c == "גורים" and df["Age"][0] < 12:
            category_products = items_df[items_df['קטגוריה'] == c]
            sl = pd.concat([sl, category_products], ignore_index=True) 
        else:    
            category_products = items_df[items_df['קטגוריה'] == c]
...
def add_image_paths(df, images_path):
    ...
    for product in df['שם מוצר']:
        ...
        if os.path.exists(image_filepath):
            image_html = f'<img src="data:image/jpg;base64,{base64.b64encode(open(image_filepath, "rb").read()).decode()}" width="100">'
...
def html_to_image(html_content):
    from weasyprint import HTML  # Make sure to have this installed: pip install WeasyPrint
    buf = BytesIO()
        ...
        with col1:
            if st.button("Download"):
                st.session_state["short list"] = (
                    sl[sl['סמן'] == True]
                    .loc[:, ["שם מוצר", "תיאור"]]
                    .assign(**{'תמונה': add_image_paths(sl[sl['סמן'] == True], "Data/Products")})
                )
